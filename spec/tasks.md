...
### 0.1 Skeleton
- [ ] Create tree from `design.md` §2 (empty modules OK).
...
### 0.2 Settings
- [ ] Implement `Settings` exactly as `design.md` §3.
- **Files:** `backend/app/settings.py`
- **Req:** 11.3 · **Constr:** C17
- **Verify:** import Settings; loads `../.env`.

### 0.3 Serializers + mock mongo
- [ ] `serialize()` ObjectId/datetime → str/ISO with offset.
- [ ] `create_mock_client()` + pymongo 4.16 bulk-update shim if needed.
- **Files:** `serializers.py`, `mock_mongo.py`

### 0.4 Signing-key guard + auth primitives
- [ ] `assert_signing_key_is_safe`, bcrypt >72 reject, JWT create/decode, CurrentUser.
- **Files:** `auth.py`
- **Req:** 1.2, 11.1, 11.3 · **Constr:** C17
- **Verify:** unit tests for password length + example secret vs remote URI.

### 0.5 Test harness
- [ ] `tests/conftest.py` blanks keys **before** app import; asserts empty secrets;
...
- [ ] `create_user`, handles, `public_user`, **allowlist** `public_member`.
...
- [ ] Routes `/api/auth/register|login|me`; lifespan connects Mongo; CORS per design §3.
- **Files:** `routers/auth.py`, `main.py`, `dependencies.py`
- **Req:** 1.1, 11.9
- **Verify:** register+login against mongomock via TestClient.

### 0.8 SPA auth shell
- [ ] `api.ts` (token + auth namespace), `auth.tsx`, `Root.tsx` guards, `SignIn`+`EnterAs`,
...
- [ ] `get_profile`, `update_profile`, `declared_profile_text`, `replace_declared_source`.
- **Constr:** C2 · **Req:** 2.1–2.4
- **Verify:** `test_declared_source.py`

### 2.2 Theme allowlist + not retrievable
- [ ] **Constr:** C3 · **Verify:** `test_profile_theme.py` keyword-stuffed background

### 2.3 Profile routes + public handle
- [ ] `GET /{handle}` uses `public_member` only.
- **Constr:** C13 · **Req:** 2.8

### 2.4 ProfileView + EditProfile + Me
- [ ] Same backdrop/card nesting; padding on shell; redirects `/agent` `/profile`.
...
- [ ] Vector cosine + keyword local; discoverable filter before rank; drop missing users.
- **Req:** 5.6, 5.7

### 3.2 RRF + match_percent + trust demote stub
- [ ] RRF_K=60; match_percent relative; trust multiplier ≤1.
- **Constr:** C6 · **Req:** 5.1, 5.4, 5.5
- **Verify:** `test_search.py`

### 3.3 Atlas probe + index script
- [ ] `probe()` real index state; `scripts/create_search_indexes.py`.
- **Constr:** C5 · **Req:** 5.3

### 3.4 Reranker
- [ ] off vs skipped; retry 429; permanent latch only on 4xx auth/validation.
- **Constr:** C8, C15 · **Verify:** `test_rerank.py`

### 3.5 Discover router + UI + StackStatus
- [ ] `/api/discover`, `/find` page, sidebar status.
- **Req:** 5.1, 11.10, 16.3
...
- [ ] Exact scoring in `design.md` §11; MAX_RESPONDERS=6; MIN=0.12.
- **Files:** `community.py` · **Req:** 4.1–4.3
- **Verify:** `test_community.py`

### 5.2 CommunityCommenter
- [ ] draft + finalize citation demotion; runtime_mode; gaps on decline.
- **Files:** `community_agent.py` · **Req:** 4.4–4.7
- **Constr:** no templated opinion

### 5.3 Community router + UI
- [ ] recruit author-only; vote recount; review_before_publish; gap demand.
- **Req:** 4.8, 4.9 · **Files:** routers + `Community.tsx`

### 5.4 InterviewAgent + store
- [ ] Per-question retrieval; batch embed questions; three decline kinds; coverage math;
...
### 7.1 ConnectionStore
- [ ] pair_id; cross-accept; permissions; provenance.
- **Req:** 3.1–3.3, 3.7 · **Verify:** `test_social.py`

### 7.2 FeedStore + ingest
- [ ] should_ingest thresholds; chunk into persona; agent templated posts; recount reactions.
- **Req:** 3.4–3.6 · **Constr:** C1 (batch embed)

### 7.3 Social router + Feed/Connections UI
- [ ] No stock member photos.
- **Constr:** C21 · **Req:** 3.8
...
- [ ] Caption rules; appearance_query_reason; consent in query; score scale; no local fallback.
- **Constr:** C7, C8 · **Req:** 6.1–6.8
- **Verify:** `test_media.py`

### 8.2 ProfileMediaStore
- [ ] Separate module; never embedded.
...
- [ ] Read **C9 in full**. Anchors = org/location/domain only.
...
- [ ] Filter `(owner_id, edge_id, space)` in query; kinds allowlist; forget on chunk delete.
...
- [ ] Runtime status; decline equal weight.
- **Req:** 16.1, 16.4
...
### 11.3 Hardening pass
- [ ] ISO timestamps with offset; CORS; discoverable defaults; JWT guard e2e.
...
- [ ] bson.json_util; copy restore into snapshot; refuse non-empty without `--force`.
- **Req:** 11.7

### 12.2 create_search_indexes + reembed docs in README comments

### 12.3 seed_users through ordinary signup path
- [ ] Non-uniform consent; no embed on startup.
- **Constr:** C1 · **Req:** 17.1–17.3

### 12.4 Confirm deleted domain stays deleted
- [ ] No `/api/agentcircle`, no LangGraph checkpointer.
- **Req:** 17.4

### 12.5 Final definition of done
- [ ] All requirements have tests or typed routes; all constraints have tests;
